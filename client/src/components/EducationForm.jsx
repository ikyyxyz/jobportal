import React from "react";
import { AiFillEdit } from "react-icons/ai";
import { BiSolidTrashAlt } from "react-icons/bi";
import { FaPlus } from "react-icons/fa";
import { HiLocationMarker } from "react-icons/hi";
import { BsClockFill } from "react-icons/bs";
import { Transition } from "@headlessui/react";
import { Fragment } from "react";
import { Dialog } from "@headlessui/react";
import { CustomButton, Loading, TextInput } from "../components";

export const EducationForm = ({
  editingEducation,
  handleEditEducation,
  userInfo,
  formatDate,
  eduDescription,
  handleDeleteEducation,
  onSubmit,
  closeModal,
  handleSubmit,
  register,
  errors,
  isSubmitting,
  open,
}) => {
  return (
    <>
      <div className="w-full md:w-2/3 2xl:w-2/4 bg-white shadow-lg p-10 pb-10 rounded-lg">
        <div className="flex items-center justify-between border border-blue-600 py-2 px-3">
          <p className="bg-[#1d4fd826] text-[#1d4fd8] py-0.5 px-1.5 rounded font-semibold text-xl">
            Education
          </p>
          {!editingEducation && (
            <button
              className="bg-yellow-400 text-white py-1 px-3 rounded"
              onClick={onSubmit}
            >
              <AiFillEdit />
            </button>
          )}
        </div>
        {editingEducation ? (
          <>
            <Transition appear show={open ?? false} as={Fragment}>
              <Dialog as="div" className="relative z-10" onClose={closeModal}>
                <Transition.Child
                  as={Fragment}
                  enter="ease-out duration-300"
                  enterFrom="opacity-0"
                  enterTo="opacity-100"
                  leave="ease-in duration-200"
                  leaveFrom="opacity-100"
                  leaveTo="opacity-0"
                >
                  <div className="fixed inset-0 bg-black bg-opacity-25" />
                </Transition.Child>

                <div className="fixed inset-0 overflow-y-auto">
                  <div className="flex min-h-full items-center justify-center p-4 text-center">
                    <Transition.Child
                      as={Fragment}
                      enter="ease-out duration-300"
                      enterFrom="opacity-0 scale-95"
                      enterTo="opacity-100 scale-100"
                      leave="ease-in duration-200"
                      leaveFrom="opacity-100 scale-100"
                      leaveTo="opacity-0 scale-95"
                    >
                      <Dialog.Panel className="w-full max-w-md transform overflow-hidden rounded-2xl bg-white p-6 text-left align-middle shadow-xl transition-all">
                        <form
                          className="w-full mt-2 flex flex-col gap-5"
                          onSubmit={handleSubmit(onSubmit)}
                        >
                          {/* Experience */}
                          <Dialog.Title
                            as="h3"
                            className="text-lg font-semibold leading-6 text-gray-900"
                          >
                            Education
                          </Dialog.Title>
                          <TextInput
                            name="eduName"
                            label="School Name"
                            placeholder="eg:Gadjah Mada University"
                            type="text"
                            register={register("eduName", {
                              required: "School Name is required",
                            })}
                            error={
                              errors.eduName ? errors.eduName?.message : ""
                            }
                          />
                          <TextInput
                            name="major"
                            label="Major"
                            placeholder="eg: Psychology"
                            type="text"
                            register={register("major", {
                              required: "Major is required",
                            })}
                            error={errors.major ? errors.major?.message : ""}
                          />
                          <TextInput
                            name="score"
                            label="Scores"
                            placeholder="eg: 80,00"
                            type="text"
                            register={register("score", {
                              required: "Scores is required",
                            })}
                            error={errors.score ? errors.score?.message : ""}
                          />
                          <TextInput
                            name="eduLocation"
                            label="Location"
                            placeholder="eg: Jogjakarta"
                            type="text"
                            register={register("eduLocation", {
                              required: "Location is required",
                            })}
                            error={
                              errors.eduLocation
                                ? errors.eduLocation?.message
                                : ""
                            }
                          />

                          <div className="w-full flex gap-2">
                            <div className="w-1/2">
                              <TextInput
                                name="eduStartDate"
                                label="From"
                                placeholder=""
                                type="date"
                                register={register("eduStartDate", {
                                  required: "This is required",
                                })}
                                error={
                                  errors.eduStartDate
                                    ? errors.eduStartDate?.message
                                    : ""
                                }
                              />
                            </div>
                            <div className="w-1/2">
                              <TextInput
                                name="eduEndDate"
                                label="To"
                                placeholder=""
                                type="date"
                                register={register("eduEndDate", {
                                  required: "This is required",
                                })}
                                error={
                                  errors.eduEndDate
                                    ? errors.eduEndDate?.message
                                    : ""
                                }
                              />
                            </div>
                          </div>

                          <div className="flex flex-col">
                            <label className="text-gray-600 text-sm mb-1">
                              Description
                            </label>
                            <textarea
                              className="ounded border border-gray-400 focus:outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500 text-base px-4 py-2 resize-none"
                              rows={4}
                              cols={6}
                              placeholder="eg: won the quiz competition"
                              {...register("eduDescription", {
                                required:
                                  "Write a little bit about yourself and your projects",
                              })}
                              aria-invalid={
                                errors.eduDescription ? "true" : "false"
                              }
                            ></textarea>
                            {errors.eduDescription && (
                              <span
                                role="alert"
                                className="text-xs text-red-500 mt-0.5"
                              >
                                {errors.eduDescription?.message}
                              </span>
                            )}
                          </div>

                          <div className="mt-4">
                            {isSubmitting ? (
                              <Loading />
                            ) : (
                              <CustomButton
                                type="submit"
                                containerStyles="inline-flex justify-center rounded-md border border-transparent bg-yellow-400 px-8 py-2 text-sm font-medium text-white hover:bg-[#ffc107] hover:text-[#fff] focus:outline-none "
                                title={"Submit"}
                              />
                            )}
                          </div>
                        </form>
                      </Dialog.Panel>
                    </Transition.Child>
                  </div>
                </div>
              </Dialog>
            </Transition>
          </>
        ) : (
          <>
            {/* {educationData.map((education, index) => (
              <div key={index}>
                <hr />
              </div>
            ))} */}
            <div className="flex flex-col items-start justify-center mb-2 mt-3">
              <h1 className="text-2xl font-semibold text-slate-600">
                {userInfo?.eduName || "No School Name"}
              </h1>

              <h5 className="text-blue-700 text-base font-bold">
                {userInfo?.major || "No major"}
              </h5>

              <div className="w-full flex flex-wrap lg:flex-row justify-start mt-2 text-sm">
                <p className="flex gap-1 items-center justify-center  px-3 py-1 text-slate-600 rounded-full">
                  <HiLocationMarker /> {userInfo?.eduLocation ?? "No Location"}
                </p>{" "}
                |
                <p className="flex gap-1 items-center justify-center px-3 py-1 text-slate-600 rounded-full">
                  <BsClockFill />
                  {formatDate(userInfo?.eduStartDate)}{" "}
                  <span className="text-black font-bold">To</span>{" "}
                  {formatDate(userInfo?.eduEndDate) || "No Date"}
                </p>
              </div>
            </div>
            <div className="w-full pb-10 pt-2">
              <div className="w-full flex flex-col-reverse md:flex-row gap-8 py-3">
                <div className="w-full md:w-full flex flex-col gap-2 text-lg text-slate-600 mt-5 md:mt-0">
                  <p className="text-[#000000]  font-semibold text-">
                    Description:
                  </p>
                  <textarea
                    className="bg-white text-sm  text-justify leading-4 border-none"
                    value={eduDescription}
                    readOnly
                    disabled
                    rows={
                      eduDescription ? eduDescription.split("\n").length : 1
                    }
                    style={{ resize: "none" }}
                  >
                    {userInfo?.eduDescription ?? "No Description Found"}
                  </textarea>
                </div>
              </div>
              {/* {!editingEducation && (
                <div className="p-1 flex gap-1">
                  <button
                    className="bg-yellow-400 text-white py-1 px-3 rounded"
                    onClick={handleEditEducation}
                  >
                    <AiFillEdit />
                  </button>
                  <button
                    className="bg-red-600 text-white py-1 px-3 rounded"
                    onClick={handleDeleteEducation}
                  >
                    <BiSolidTrashAlt />
                  </button>
                </div>
              )} */}
            </div>
          </>
        )}
      </div>
    </>
  );
};
